...
- **Repository governance:** See [AGENTS.md](../../AGENTS.md) for toolchain requirements, nullability policy, and Gradle
  invocation rules.
- **General layout:** [contributor-guidelines.md](general/contributor-guidelines.md) defines production vs. test
  directories and outlines writing documentation inside `context/`.
- **iCLI specifics:** [project-conventions.md](icli/project-conventions.md) explains package prefixes, PTY usage
  defaults, and virtual-thread expectations.
- **Modularity principles:** [assistant-notes.md](icli/assistant-notes.md) captures the preferred decomposition patterns
  (records for immutable data, assistant-centric TDD ritual) that keep component scopes small enough for reliable AI
  collaboration.

## 2. Coding standards
- **Mandatory rules:** [coding/standards.md](coding/standards.md) is the canonical reference for Palantir Java Format,
  Kotlin conventions, nullability, immutability, and record usage.
...
- **Follow-up tooling:** When altering build or formatting configuration, consult
  [workflow/quality-gates.md](../workflow/quality-gates.md) for the full automation checklist and update this overview
  if new requirements arise.

## 4. Testing expectations
- **Strategy baseline:** [testing/strategy.md](../testing/strategy.md) explains unit vs. integration coverage, PTY
...
- **Living knowledge:** [context/README.md](../README.md) and the [context overview](../context-overview.md) describe
  how knowledge is organised. Follow the overview before every task to refresh mandatory materials.
- **Roadmap alignment:** [project-roadmap.md](../roadmap/project-roadmap.md) and its companion briefs (execution
  requirements, architecture, use cases) drive prioritisation. Update them when behaviour or scope shifts.
- **Task history:** Maintain dossiers in `context/tasks/` per [tasks/README.md](../tasks/README.md). Every delivery must
  log planning, analysis, execution, and retrospectives so future assistants can reconstruct decisions quickly.
...
- **Branching & reviews:** [workflow/collaboration.md](../workflow/collaboration.md) lists branch naming, pull-request
  evidence, and Definition of Done expectations.
- **Release readiness:** Track release steps in [workflow/releases.md](../workflow/releases.md); expand it as the
...
- **Session close-out:** Follow [checklists/session-completion.md](../checklists/session-completion.md) before ending a
  task. Record completion inside the active dossier.

## 7. Process improvement & future work
- **Guidelines rollout tracker:** See [../checklists/guidelines-rollout.md](../checklists/guidelines-rollout.md) for the
  status of guideline initiatives and outstanding backlog items.
- **Assistant needs:** When new documents emerge, update this overview and the rollout checklist simultaneously to keep
  navigation reliable.
- **Research linkage:** Register studies in [research/registry.md](../research/registry.md) so assistants can discover
  experiments relevant to forthcoming tasks.
