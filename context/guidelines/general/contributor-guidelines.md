...
Guidelines Overview](/context/guidelines/project-overview.md) for navigation, then use the sections below to clarify
day-to-day behaviour. Project-specific rules continue to live under
[`context/guidelines/icli/`](/context/guidelines/icli).

## 1. Orientation

- Read the root [AGENTS.md](/AGENTS.md) before contributing and add scoped agents when new modules demand extra
  constraints.
- Treat [context/README.md](/context/README.md) and [context/context-overview.md](/context/context-overview.md) as the
  knowledge entry points. The overview lists mandatory documents to revisit at the start of every session.
- Follow the assistant-managed workflow described in [context/tasks/README.md](/context/tasks/README.md); maintain task
  dossiers so history stays discoverable.
- Respect the single-maintainer model in [project-roles.md](/context/guidelines/icli/project-roles.md) and keep all
  documentation in English.
- Apply the Markdown rules in [markdown-formatting.md](/context/guidelines/general/markdown-formatting.md) when editing
  `.md` files.
...
- When introducing new top-level directories or modules, add a scoped [AGENTS.md](/AGENTS.md) and reference it from the
  project overview to help assistants discover the rules.
...
- Target Java 25 in production and adhere to [coding/standards.md](/context/guidelines/coding/standards.md) for Palantir
  Java Format, Spotless enforcement, nullability, records, and immutability.
...
- Follow the [testing strategy](/context/testing/strategy.md) for unit vs. integration coverage, PTY vs. non-PTY
  matrices, timeout discipline, and cross-platform goals.
...
- Update architecture docs, READMEs, and knowledge base entries in lockstep with code changes. Register new research via
  [context/research/registry.md](/context/research/registry.md).
- Follow [workflow/collaboration.md](/context/workflow/collaboration.md) for branch naming, PR evidence, and Definition
  of Done expectations. Add automation checkpoints to execution logs.
- Consult [workflow/releases.md](/context/workflow/releases.md) before tagging a release and record platform validation
  results.
...
- Close each session by running the checklist in
  [context/checklists/session-completion.md](/context/checklists/session-completion.md) and logging completion inside
