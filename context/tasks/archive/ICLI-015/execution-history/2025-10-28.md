...
## Links
- `context/tasks/ICLI-015/README.md`
...
## Retrospective & process improvement
- Deterministic schedulers materially improved test speed and reliability; future timeout-related work should follow
  the same pattern before introducing real sleeps.
...
- **Author:** Assistant
- **Lifecycle stage:** Execution
- **Summary:** Applied `pool-refactoring-3.md` P0 fixes by eliminating the timeout race, enforcing FIFO fairness when
  idle workers exist, and preventing minimum-size replenishment after `close()`. Expanded unit coverage to lock these
  behaviours down.
...
- Updated `PoolState.acquire` to hand idle workers to queued waiters, preserving FIFO order even under contention, and
  added lifecycle guards around `reserveForMinimum` so closing pools stop launching replacements
  (`PoolState.java:62`, `PoolState.java:229`).
...
## Documentation updates
- Recorded the execution plan under `context/tasks/ICLI-015/execution-history/2025-10-28-plan.md`.
...
- **Author:** Assistant
- **Lifecycle stage:** Execution
- **Summary:** Applied pool-refactoring-4 recommendations by tightening worker inventory invariants, improving
  deadline handling, and capturing the updates in the pooled source bundle.

## Implementation notes
- Hardened `CapacityLedger` bookkeeping with assertion-guarded decrements and a shared helper that records retired
  idle workers while preserving invariant clarity
...
- Reordered lease deadline cancellation to occur before removing the active lease entry and trimmed redundant waiter
  removals, matching the race fixes described in `pool-refactoring-4.md`
  (`src/main/java/com/github/ulviar/icli/core/pool/ProcessPool.java`, `PoolState.java`).
...
## Follow-up & risks
- Optional clean-ups from `pool-refactoring-4.md` (single invariant assertions per method, diagnostics debouncing)
  remain candidates for later iterations.
