...
1. **Run the automated gate.** Execute `python scripts/pre_response_checks.py`. Follow every instruction it prints: this
   covers `git status`, canonical change detection (`scripts/list_changed_files.py`), Markdown formatting, and
   verification that `.commit-message` is populated.
2. **Trigger Gradle workflows via MCP.** When the script indicates source files changed, run `execute_gradle_task` with
