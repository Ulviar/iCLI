...
| Stateful pooled conversations                           | Direct sessions via `CommandService` when a single worker suffices                               | `ServiceConversation` / `PooledLineConversation` via `service.pooled().client(...)`               | Use `ConversationAffinity` to hint worker reuse and `ConversationReset`/`ConversationRetirement` to surface health diagnostics while rotating unhealthy workers. |
...
- `PooledClientSpec` — fluent configurator for pool sizing, diagnostics listeners, and idle policies used by the pooled
  facade helpers.
- `ConversationAffinity`, `ConversationReset`, `ConversationRetirement` — metadata helpers that let conversations hint
  worker reuse and describe manual reset/retirement reasons for diagnostics.
